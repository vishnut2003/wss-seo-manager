...
export async function updateDailySummarySettings(
  ...
  revalidatePath(`/projects/${projectId}/notifications/daily-summary`);
  return { ok: true };
}

export async function sendTestSummary(projectId: string): Promise<ActionResult> {
  const session = await auth();
  if (!session?.user) return { ok: false, error: "Not authenticated" };
  if (!isManager(session.user.role)) {
  ...
  try {
    const { status } = await runDailySummary(projectId, { force: true });
    if (status !== "sent") {
