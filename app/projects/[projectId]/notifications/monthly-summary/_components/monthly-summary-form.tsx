...
export function MonthlySummaryForm({
  ...
  async function onSave() {
    setSaving(true);
    const res = await updateMonthlySummarySettings(projectId, {
      enabled,
      recipients,
  ...
  async function onTest() {
    setTesting(true);
    const res = await sendTestMonthlySummary(projectId);
    setTesting(false);
      ...
      <CardHeader>
        ...
        <CardDescription>
          {canManage
            ...
            : "Monthly summary configuration (read-only)."}
        </CardDescription>
      </CardHeader>
        ...
        <div className="flex items-center justify-between gap-4 rounded-xl border border-purple-100 bg-purple-50/40 p-4">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Send the digest automatically every month.
            </p>
          </div>
          <Switch
            checked={enabled}
